export default function Footer() {
  return (
    <footer className="py-8 md:py-12 px-6 bg-[#2D2416]">
      <div className="max-w-6xl mx-auto">

        {/* Mobile : colonne compacte / Desktop : ligne */}
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 md:gap-6">

          {/* Marque */}
          <div className="flex items-center justify-between md:block">
            <div>
              <span className="font-[family-name:var(--font-serif)] text-[#F7F0E6] text-lg md:text-xl block mb-0.5">
                Soleia Massage
              </span>
              <p className="text-[#7A6652] text-[10px] tracking-widest uppercase">
                par Alexanne
              </p>
            </div>
            {/* Instagram visible uniquement sur mobile, ici à droite du nom */}
            <a
              href="https://www.instagram.com/soleia.massage.alexanne"
              target="_blank"
              rel="noopener noreferrer"
              aria-label="Instagram Soleia Massage"
              className="md:hidden text-[#7A6652] hover:text-[#C06040] transition-colors"
            >
              <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <rect x="2" y="2" width="20" height="20" rx="5" />
                <circle cx="12" cy="12" r="4" />
                <circle cx="17.5" cy="6.5" r="0.5" fill="currentColor" />
              </svg>
            </a>
          </div>

          {/* Nav links — 2 colonnes sur mobile, ligne sur desktop */}
          <div className="grid grid-cols-2 md:flex md:gap-8 gap-x-6 gap-y-2 text-[#7A6652] text-sm">
            {[
              { label: "Services", href: "#services" },
              { label: "À propos", href: "#about" },
              { label: "Témoignages", href: "#testimonials" },
              { label: "Contact", href: "#contact" },
            ].map((link) => (
              <a key={link.label} href={link.href} className="hover:text-[#C06040] transition-colors">
                {link.label}
              </a>
            ))}
          </div>

          {/* Instagram + copyright — masqué sur mobile (Instagram déjà en haut) */}
          <div className="hidden md:flex items-center gap-4">
            <a
              href="https://www.instagram.com/soleia.massage.alexanne"
              target="_blank"
              rel="noopener noreferrer"
              aria-label="Instagram Soleia Massage"
              className="text-[#7A6652] hover:text-[#C06040] transition-colors"
            >
              <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <rect x="2" y="2" width="20" height="20" rx="5" />
                <circle cx="12" cy="12" r="4" />
                <circle cx="17.5" cy="6.5" r="0.5" fill="currentColor" />
              </svg>
            </a>
            <p className="text-[#7A6652] text-xs">© 2026 Soleia Massage</p>
          </div>
        </div>

        {/* Bas : copyright mobile + GitHub */}
        <div className="mt-6 pt-4 border-t border-[#F7F0E6]/5 flex items-center justify-between">
          <p className="md:hidden text-[#7A6652] text-xs">© 2026 Soleia Massage</p>
          <a
            href="https://github.com/RustyRory/"
            target="_blank"
            rel="noopener noreferrer"
            className="ml-auto flex items-center gap-1.5 text-[#7A6652]/40 hover:text-[#7A6652]/70 transition-colors text-[10px] tracking-widest uppercase"
          >
            <svg className="w-3 h-3" viewBox="0 0 24 24" fill="currentColor">
              <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0 1 12 6.844a9.59 9.59 0 0 1 2.504.337c1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.02 10.02 0 0 0 22 12.017C22 6.484 17.522 2 12 2z" />
            </svg>
            RustyRory
          </a>
        </div>
      </div>
    </footer>
  );
}
